...
interface DayOffFormProps {
  ...
  loading: boolean;
  onSubmit: (input: { barberId: string; date: string; reason: string }) => Promise<void>;
}

export function DayOffForm({ selectedBarber, loading, onSubmit }: DayOffFormProps) {
  ...
  return (
    <form className="rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--surface-2))] p-4 space-y-2 shadow-[var(--shadow-soft)]" onSubmit={handleSubmit}>
      ...
      <input type="text" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Motivo" className="w-full rounded-2xl border border-[rgb(var(--border))] px-3 py-2" />
