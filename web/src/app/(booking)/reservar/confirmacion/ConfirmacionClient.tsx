...
export default function ConfirmacionClient({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // transición pro: spinner → success circle → ticket
        setState("success");
        setTimeout(() => {
  ...
  const barberName = useMemo(() => {
    if (!booking) return "";
    return (
      BARBERS.find((b) => b.id === booking.barberId)?.name ?? booking.barberId
    );
  }, [booking]);
    ...
    <div className="space-y-5">
      ...
      <div className="relative z-20 grid grid-cols-2 gap-3">
        <button
          type="button"
          onClick={() => router.push("/")}
          className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--surface))] px-4 py-3 text-sm font-semibold hover:brightness-110 active:scale-[0.99]"
        >
          Ir al inicio
        </button>
        ...
        <button
          ...
          className="rounded-2xl bg-[rgb(var(--primary))] px-4 py-3 text-sm font-semibold text-[rgb(var(--primary-foreground))] hover:brightness-110 active:scale-[0.99]"
        >
          Reservar otra
        </button>
      </div>
...
function Section({
  ...
  return (
    <div className="p-4 flex gap-4">
      <div className="h-12 w-12 rounded-2xl border border-[rgb(var(--border))] bg-black/5 dark:bg-white/5 grid place-items-center">
        {icon}
      </div>
