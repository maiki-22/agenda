...
export function ReservarClient({ initialCatalog }: ReservarClientProps) {
  ...
  const { services, barbers } = useCatalog(initialCatalog);
  ...
  async function onConfirm() {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ["booking-catalog"] });
  ...
  const barberNameFromId = (id: string) => barbers.find((b) => b.id === id)?.name ?? id;
